...
def train():
    print("=" * 50)
    print("NAMEHA LSTM TRAINING — IMPROVED")
    print("=" * 50)

    print("\n📂 Loading improved data...")
    X, y = load_all_data()

    if len(X) == 0:
        print("❌ No data loaded. Run improve_data.py first.")
        return
    ...
    # Instead of removing data, we use class weights
    # This tells the model "pay more attention to up days
    ...
    class_weight = {
        0: total / (2 * down_count),  # weight for down days
        1: total / (2 * up_count)     # weight for up days
    }

    print(f"\n⚖️  Using class weights instead of removing data:")
    ...
    patience=15,             # was 10 — give it more time
    ...
    # Save
    model_path = os.path.join(MODEL_FOLDER, "lstm_nameha.keras")
    ...
    with open(os.path.join(MODEL_FOLDER, "model_stats.json"), "w") as f:
        json.dump(stats, f, indent=2)

    print(f"   Stats saved to model/model_stats.json")

    # Plot
    print("\n📊 Saving training chart...")
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(MODEL_FOLDER, "training_chart.png"))
    print(f"   Chart saved to model/training_chart.png")
    ...
    if accuracy >= 0.60:
        print("   ✅ Excellent — strong model for Nameha")
    elif accuracy >= 0.55:
        print("   ✅ Good — usable model for Nameha")
    else:
